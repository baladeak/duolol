...
router.patch('/me', auth, async (req, res) => {
  ...
  if (lol_game_name !== undefined) {
    const safeName = (lol_game_name || '').trim().slice(0, 20) || null;
    await db.execute('UPDATE users SET lol_game_name=? WHERE id=?', [safeName, req.user.id]);
  }
